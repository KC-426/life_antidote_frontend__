...
function EditBlogs({ handleClose,blogIdForEdit }) {
  const [blogsData, setBlogsData] = useState({blog_category:""});
  ...
  console.log("blogIdForEdit DATA", blogIdForEdit);
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/admin/get/single/blog/by/id/${blogIdForEdit}`,
        { withCredentials: true }
      )
      .then((res) => {
        console.log("setBlogsData==>>>>",res?.data);
        setBlogsData(res?.data?.data);
        setBlogSubHeadingField(res?.data?.data?.blog_sub_headings);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const handleSubmit = async (e) => {
    ...
    // if (!fileUpload && !blogsData?.blog_image?.image_url ) {
    ...
    if (fileProfileUpload) {
       ...
       console.log("FINAL blogsData ====>>>",blogsData);
    await axios
      .patch(
        `${process.env.REACT_APP_BACKEND_URL}/api/admin/edit/blog/by/id/${blogIdForEdit}`,
      ...
      <div className="product-conatiner">
        <div className="addproducts_slider">
          <div className="slider_heading">
            <h4>Edit Blog</h4>
            <p>Edit your blog and necessary information from here</p>
          </div>
          ...
          <div className="addproduct_img_and_details flex">
            <div className="file_upload_col">
              ...
              {(!fileUpload?.length && blogsData?.blog_image?.image_url) &&
                    <div  className="uploaded-images-preview">
                      <img
                        className="category-table-image"
                        alt="product"
                        src={blogsData?.blog_image?.image_url}
                      />
                      <p>{blogsData?.blog_image?.image_name}</p>
             ...
             <h3>Author Details</h3>
              ...
              <div className="add_product_label_input">
                ...
                <img
                  ...
                  //     : noImage
                  // }
                  src={blogsData?.author_profile?.image_url
                    ? blogsData?.author_profile?.image_url
                    :fileProfileUpload
                    ? URL.createObjectURL(fileProfileUpload)
                    : noImage
                  }
                />
